...
class rsnotify extends CI_Controller{
    ...
    public function index(){
    	//状态栏控制
    	$data['title'] = '人事通知管理';
    	$data['navshow'] = 'rsnotify';
        //导航栏控制
        $breadcrumb = '<h3 class="page-title">管理员管理后台<small>人事通知管理</small></h3>';
        $breadcrumb .= '<ul class="breadcrumb">';
        
        $breadcrumb .= '<li>';
        $breadcrumb .= '<i class="icon-home"></i>';
        $breadcrumb .= '<a href="/index.php?c=rsnotify">首页</a>';
        $breadcrumb .= '<i class="icon-angle-right"></i>';
        $breadcrumb .= '</li>';
        
        
        $breadcrumb .= '<li>';
        $breadcrumb .= '<a href="/index.php?c=rsnotify">人事通知管理</a>';
        $breadcrumb .= '<i class="icon-angle-right"></i>';
        $breadcrumb .= '</li>';
        
        $breadcrumb .= '<li>人事通知列表</li>';
        $breadcrumb .= '</ul>';
        $data['breadcrumb'] = $breadcrumb;
    	//权限控制
    	$data['shownavlist'] = $this -> checkuser -> system_level($data['navshow']);
    	$this -> checkuser -> checkrsnotify();
        $this -> load -> view('showrsnotify', $data);
    }
    ...
    public function ajax_data(){
        ...
        //获取数据总条数
        $sql_notify_total = "select count(id) as memTotal from oa_notify where whose='3'";
        ...
        //查询条件
        $sql_notify .= " where n.whose='3' "; 
        if($search){
            $sql_notify .= " and (n.whose='3' and n.id like '%{$search}%' or u.fullname like '%{$search}%' or n.send_dept like '%{$search}%' or n.title like '%{$search}%') ";
        }
        //按对应字段排序
        switch ($sortcol) {
            case '1':
        ...
        for ($i=0; $i < $num_memData; $i++){
            ...
            if($reAjaxData['aaData'][$i][6] == '未发送'){
            	$reAjaxData['aaData'][$i][] = '<a href="/index.php?c=rsnotify&m=details&id='.$id.'"><span class="label label-success">查看详情</span></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href ="/index.php?c=rsnotify&m=send&id='.$id.'" onclick="javascript:return confirm(\'确定要发送吗?\')"."><span class="label label-success">发送通知</span></a>';
            }else{
            	$reAjaxData['aaData'][$i][] = '<a href="/index.php?c=rsnotify&m=details&id='.$id.'"><span class="label label-success">查看详情</span></a>';
            }
        }
...
//         $data['users'] = $users;
        ...
        $data['user'] = $user;
        $this -> load -> view('addrsnotify',$data);
    }
...
//         $sender = trim($_POST['sender_id']) ? trim($_POST['sender_id']) : '';
        ...
        if($notify_type == 1){
        	...
        	if(!empty($team) && empty($user)){
        		...
        		echo "<script>alert('请填写通知联系组或个人');parent.location.href='index.php?c=rsnotify&m=add';</script>";exit;
        	}
        }
		...
		if(!empty($_FILES['img']['tmp_name'])){
			...
			if( ! $this->upload->do_upload('img')){
				echo "<script>alert('图片大小超出5M或上传失败');parent.location.href='index.php?c=rsnotify&m=add';</script>";exit;
			}else{
        ...
        if($notify_type == 1){
        	...
        	$sql_notify = "insert into oa_notify(sender_id, send_dept, notify_type, notify_man, listid, title, content, time, img, whose) values('$sender_id', '$send_dept', '$notify_type', '$notify_man ', '$listidstr', '$title', '$contents', '$time', '$img', '3')";
        	...
        	if($obj_notify && $ob_notify_senduser){
        		echo "<script>alert('通知添加成功!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
        	}else{
        		echo "<script>alert('通知添加失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
        	}
        }else{
    ...
    public function edit(){
        ...
        $user = $ob_user -> result_array();
        $data['user'] = $user;
        $this -> load -> view('editrsnotify',$data);
    }
    ...
    public function update(){
        ...
        if($notify_type == 1){
        	...
        	if(!empty($team) && empty($user)){
        		...
        		echo "<script>alert('请填写通知联系组或个人');parent.location.href='index.php?c=rsnotify&m=edit&id=$id';</script>";exit;
        	}
        }
        $title = trim($_POST['title']) ? trim($_POST['title']) : "";
        $contents = trim($_POST['content']) ? trim($_POST['content']) : "";
...
//         			return false;
        			...
        			echo "<script>alert('".$res."');parent.location.href='index.php?c=rsnotify&m=edit&id=$id';</script>";
        		}else{
        			//处理成功的推送
        			$sql_notify = "update oa_notify set sender_id='$sender_id', notify_type='$notify_type', notify_man='$notify_man', listid='$listid', title='$title', content='$contents', time='$time', img='$img', state='2', whose='3' where id='$id'";
					$obj_notify = $this -> db -> query($sql_notify);
        			if($obj_notify){
        				if(!empty($newimg)){
        				...
        				echo "<script>alert('通知成功!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
        			}else{
        				echo "<script>alert('通知成功,写入数据库失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
        			}
        			
        		}
        	}else{      //接口调用失败或无响应
        		echo "<script>alert('通知失败!');parent.location.href='index.php?c=rsnotify&m=edit&id=$id';</script>";
        		return false;
        	}
    }
    ...
    public function del(){
        ...
        if($ob_notify){
        	...
        	@unlink($_SERVER['SINASRV_DATA_DIR'].$img);
            echo "<script>alert('删除成功!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
        }else{
            echo "<script>alert('删除失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
        }
    }
    ...
    public function details(){
        ...
        $breadcrumb .= '<li>人事通知详情列表</li>';
        $breadcrumb .= '</ul>';
        $data['breadcrumb'] = $breadcrumb;
    	//权限控制
    	$data['shownavlist'] = $this -> checkuser -> system_level($data['navshow']);
    	$this -> checkuser -> checkrsnotify();
    	$this -> load -> view('showrsnotify_details', $data);
    }
    		...
    		if(!empty($notify)){
    			...
    			if(empty($sendlist)){
    				...
    				if($ob_state && $ob_notify){
    					...
    					echo "<script>alert('通知成功,数据库写入失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
    				}
    			}else{
    				//使用推送
    				$pushObj = new Jpush();
    				...
    				if($result){
    					...
    					if(isset($res_arr['error'])){                       //如果返回了error则证明失败
    						...
    						$ob_notify = $this -> db -> query($sql_notify);
    						if($ob_notify){
    							echo "<script>alert('通知失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
    						}else{
    							echo "<script>alert('通知失败,数据库写入失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
    						}
    						//     					$res = $res_arr['error']['message'];
    					...
    					echo "<script>alert('通知推送失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
    				}
    			}
    		}else{
    			echo "<script>alert('获取通知信息失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
    		}
    
    	}else{
    		echo "<script>alert('获取通知id失败!');parent.location.href='index.php?c=rsnotify&m=index';</script>";
    	}
    }
}
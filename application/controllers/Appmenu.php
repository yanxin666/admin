...
class appmenu extends CI_Controller{
    ...
    public function index(){
    	//状态栏控制
    	$data['title'] = '乐居菜单管理';
    	$data['navshow'] = 'appmenu';
        //导航栏控制
        $breadcrumb = '<h3 class="page-title">管理员管理后台<small>乐居菜单管理</small></h3>';
        $breadcrumb .= '<ul class="breadcrumb">';
        
        $breadcrumb .= '<li>';
        $breadcrumb .= '<i class="icon-home"></i>';
        $breadcrumb .= '<a href="/index.php?c=appmenu">首页</a>';
        $breadcrumb .= '<i class="icon-angle-right"></i>';
        $breadcrumb .= '</li>';
        
        
        $breadcrumb .= '<li>';
        $breadcrumb .= '<a href="/index.php?c=appmenu">乐居菜单管理</a>';
        $breadcrumb .= '<i class="icon-angle-right"></i>';
        $breadcrumb .= '</li>';
        
        $breadcrumb .= '<li>乐居菜单列表</li>';
        $breadcrumb .= '</ul>';
        $data['breadcrumb'] = $breadcrumb;
    	//权限控制
    	$data['shownavlist'] = $this -> checkuser -> system_level($data['navshow']);
    	$this -> checkuser -> checkappmenu();
        $this -> load -> view('showappmenu', $data);
    }
    ...
    public function ajax_data(){
        ...
        //获取数据总条数
        $sql_appmenu_total = "select count(id) as memTotal from oa_appmenu";
        $memTotal_ob = $this -> db -> query($sql_appmenu_total);
        ...
        //查询数据
        $sql_member = "select id, title, status, createtime from oa_appmenu";
        //查询条件
        if($search){
        ...
        for ($i=0; $i < $num_memData; $i++){
            ...
            $reAjaxData['aaData'][$i][] = '<a href="/index.php?c=appmenu&m=edit&id='.$id.'"><span class="label label-success">修改</span></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href ="/index.php?c=appmenu&m=del&id='.$id.'" onclick="javascript:return confirm(\'删除后无法恢复,确定要删除吗?\')"."><span class="label label-success">删除</span></a>';
        }
    ...
    public function add(){
        ...
        $breadcrumb .= '<li>乐居菜单添加</li>';
        $breadcrumb .= '</ul>';
        $data['breadcrumb'] = $breadcrumb;
        //权限控制
        $data['shownavlist'] = $this -> checkuser -> system_level($data['navshow']);
        $this -> checkuser -> checkappmenu();
        $this -> load -> view('addappmenu',$data);
    }
    ...
    public function insert(){
        ...
        $sql_appmenu = "insert into oa_appmenu(title, status, createtime) values('$title', '$status', '$createtime')";
        $obj_appmenu = $this -> db -> query($sql_appmenu);
        //判断添加是否成功
        if($obj_appmenu){
            echo "<script>alert('添加成功!');parent.location.href='index.php?c=appmenu&m=index';</script>";
        }else{
            echo "<script>alert('添加失败!');parent.location.href='index.php?c=appmenu&m=index';</script>";
        }
    }
    ...
    public function edit(){
        ...
        $sql_appmenu = "select id, title, status from oa_appmenu where id={$id};";
        $appmenu_ob = $this -> db -> query($sql_appmenu);
        $appmenu = $appmenu_ob -> row_array();
        $data['appmenu'] = $appmenu;
        $this -> load -> view('editappmenu',$data);
    }
    ...
    public function update(){
        ...
        $status = is_numeric($_POST['status']) ? intval($_POST['status']) : 0;
        $createtime = date('Y-m-d H:i:s',time());
        $sql_appmenu = "update oa_appmenu set title='$title',status='$status',createtime='$createtime' where id='$id'";
        $ob_appmenu = $this -> db -> query($sql_appmenu);
        //判断是否修改成功
        if($ob_appmenu){
            echo "<script>alert('修改成功!');parent.location.href='index.php?c=appmenu&m=index';</script>";
        }else{
            echo "<script>alert('修改失败!');parent.location.href='index.php?c=appmenu&m=index';</script>";
        }
    }
    ...
    public function del(){
        $id = is_numeric($_GET['id']) ? intval($_GET['id']) : "";
        $sql_appmenu ="delete from oa_appmenu where id='$id'";
        $ob_appmenu = $this -> db ->query($sql_appmenu);
        //判断是否删除成功
        if($ob_appmenu){
            echo "<script>alert('删除成功!');parent.location.href='index.php?c=appmenu&m=index';</script>";
        }else{
            echo "<script>alert('删除失败!');parent.location.href='index.php?c=appmenu&m=index';</script>";
        }
    }
    ...
    function dellist(){
    	...
    	$sql_del = "delete from oa_appmenu where id in ($idlist)";
    ...
    public function check(){
        ...
        $sql_appmenu = "select * from oa_appmenu where title='$title'";
        $appmenu_ob = $this -> db -> query($sql_appmenu);
        $appmenus = $appmenu_ob -> row_array();
        //判断网址是否合法
        if(empty($appmenus)){
            echo "true";exit;
        }else{
    ...
    public function checkedit(){
        ...
        if($newtitle == $oldtitle){
            ...
            $sql_title = "select * from oa_appmenu where title='$newtitle';";

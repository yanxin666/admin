...
//                "bAutoWidth": true,																						
                ...
                //使用ajax源
                "sAjaxSource": "/index.php?c=qtnotify&m=ajax_data",
            ...
            jQuery('#sample_qtnotify_1 .group-checkable').change(function () {
 ...
 */
 ...
 if (str.length > 0) {
     ...
     if(confirm('删除后无法恢复,确定要删除吗?') == true){
    	 $.ajax({
        	 url: "/index.php?c=qtnotify&m=dellist",
        	 data: "idlist="+idlist,
        	 type: "post",
        	 async: true,
        	 success: function(data){
        		 if(data == 1){
        			 alert("删除成功");
        			 window.location.href="/index.php?c=qtnotify&m=index";
        		 }else{
        			 alert("删除失败");
        			 window.location.href="/index.php?c=qtnotify&m=index";
        		 }

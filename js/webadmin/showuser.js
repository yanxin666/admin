jQuery(document).ready(function() {
    ...
    // begin first table
            $('#sample_user_1').dataTable({
                ...
                "aLengthMenu": [
                    [50, 70, 90, 100, -1],
                    [50, 70, 90, 100, "全部"]   //每页变化值
                ],
                "iDisplayLength": 50,    //设置datatables的默认显示条数
                // "iColumns": 11
            ...
            jQuery('#sample_user_1 .group-checkable').change(function () {
 ...
 */
 ...
 if (str.length > 0) {
     ...
     if(confirm('删除后无法恢复,确定要删除吗?') == true){
    	 $.ajax({
        	 url: "/index.php?c=user&m=dellist",
        	 data: "idlist="+idlist,
        	 type: "post",
        	 async: true,
        	 success: function(data){
        		 if(data == 1){
        			 alert("删除成功");
        			 window.location.href="/index.php?c=user&m=index";
        		 }else{
        			 alert("删除失败");
        			 window.location.href="/index.php?c=user&m=index";
        		 }

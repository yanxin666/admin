jQuery(document).ready(function() {
    ...
    // begin first table
            ...
            jQuery('#sample_expense_1 .group-checkable').change(function () {
 ...
 */
 ...
 if (str.length > 0) {
     ...
     if(confirm('删除后无法恢复,确定要删除吗?') == true){
    	 $.ajax({
        	 url: "/index.php?c=expense&m=dellist",
        	 data: "idlist="+idlist,
        	 type: "post",
        	 async: true,
        	 success: function(data){
        		 if(data == 1){
        			 alert("删除成功");
        			 window.location.href="/index.php?c=expense&m=index";
        		 }else{
        			 alert("删除失败");
        			 window.location.href="/index.php?c=expense&m=index";
        		 }
